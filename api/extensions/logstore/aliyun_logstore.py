...
class AliyunLogStore:
    ...
    def __init__(self) -> None:
        ...
        # Append CheersAI identification to the existing user agent
        original_user_agent = self.client._user_agent  # pyright: ignore[reportPrivateUsage]
        dify_version = dify_config.project.version
        enhanced_user_agent = f"CheersAI,CheersAI-{dify_version},{original_user_agent}"
    ...
    def create_logstore_if_not_exist(self) -> None:
        ...
        for logstore_name in logstore_name_list:
            ...
            # Ensure index contains all CheersAI-required fields
    ...
    def _merge_index_configs(
        ...
        """
        Intelligently merge existing index config with CheersAI's required field indexes.

        This method:
        1. Preserves all existing field indexes in logstore (including custom fields)
        2. Adds missing CheersAI-required fields
        3. Updates fields where type doesn't match (with json/text compatibility)
        4. Corrects case mismatches (e.g., if CheersAI needs 'status' but logstore has 'Status')

        Type compatibility rules:
        - json and text types are considered compatible (users can manually choose either)
        - All other type mismatches will be corrected to match CheersAI requirements
        ...
        Args:
            ...
            required_keys: CheersAI's required field index configurations
        ...
        if missing_fields:
            logger.info(
                "Logstore %s: Adding %d missing CheersAI-required fields: %s",
                logstore_name,
                len(missing_fields),
    ...
    def ensure_index_config(self, logstore_name: str) -> None:
        """
        Ensure index configuration includes all CheersAI-required fields.

        This method intelligently manages index configuration:
        1. If index doesn't exist, create it with CheersAI's required fields
        2. If index exists:
           - Check if all CheersAI-required fields are present
        ...
        """
        # Get CheersAI's required field indexes
        required_keys = {}
        if logstore_name == AliyunLogStore.workflow_execution_logstore:
        ...
        if existing_config is None:
            ...
            if needs_update:
                logger.info("Logstore %s: Updating index to include CheersAI-required fields", logstore_name)
                try:
                ...
                logger.info(
                    "Logstore %s: Index already contains all %d CheersAI-required fields with correct types, "
                    "no update needed",
                    logstore_name,
                    len(required_keys),
                )

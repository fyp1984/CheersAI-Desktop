...
def test_pdf_export():
    ...
    try:
        output_path = "测试文档.pdf"
        result = markdown_to_pdf(test_markdown_content(), output_path)
        
        if os.path.exists(output_path):
            file_size = os.path.getsize(output_path)
            print(f"✅ PDF 导出成功: {output_path} ({file_size} bytes)")
            return True
        else:
        ...
        print("提示: PDF 生成需要系统依赖，请参考 INSTALLATION.md")
        import traceback
        traceback.print_exc()
        return False
...
def main():
    ...
    if passed == total:
        print("\n🎉 所有测试通过！插件功能正常。")
        print("\n生成的文件:")
        for filename in ["测试文档.docx", "测试文档.pdf", "测试文档.html", "测试文档.md"]:

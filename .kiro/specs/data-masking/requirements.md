...
数据脱敏功能是在基于 CheersAI 平台的桌面应用中新增的本地数据保护能力。用户可以在本地桌面客户端对敏感数据进行脱敏处理，将脱敏后的文件存储在本地沙箱环境中，然后上传到 CheersAI 云端后端进行 AI 处理，并在返回结果时自动恢复原始数据。这确保了端到端的数据隐私，同时充分利用 CheersAI 云端 AI 能力。

## 项目架构说明

本项目基于 CheersAI 开源平台进行定制开发：

- **服务器端**: 使用 CheersAI 后端（Python Flask + DDD 架构 + Celery + Redis）
- **桌面客户端**: 使用 CheersAI 前端（Next.js + TypeScript + React）+ **新增数据沙箱功能**
- **核心新增功能**: 本地数据脱敏和沙箱管理（完全在桌面客户端实现）
- **部署方式**: Docker 容器化部署

数据脱敏和沙箱功能是桌面客户端的新增模块，不需要修改 CheersAI 后端的核心逻辑，仅需要后端配合进行文件路径验证。

## 术语表

- **桌面客户端（Desktop_Client）**: 基于 CheersAI 前端（Next.js + TypeScript + React）的本地桌面应用程序，新增数据脱敏和沙箱功能
...
- **Dify_后端（Dify_Backend）**: 处理脱敏文件的 CheersAI 云服务后端（Python Flask + DDD）
...
**用户故事：** 作为用户，我希望将沙箱中的脱敏文件上传到 CheersAI 后端，以便在不暴露原始敏感数据的情况下利用 AI 处理能力。
...
**用户故事：** 作为用户，我希望系统自动恢复从 CheersAI 后端返回结果中的原始敏感数据，以便无需手动干预即可查看完整准确的信息。
...
**用户故事：** 作为系统管理员，我希望 CheersAI 后端仅访问用户配置的沙箱路径中的文件，以便系统强制执行安全边界并防止未经授权的文件访问。
